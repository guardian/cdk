...
const parseOtherConstructors = (context, node) => {
  ...
  if (scope.name !== "scope") {
    return context.report(
      node,
      scope.loc,
      `The first parameter in a construct or pattern constructor must be called scope`
    );
  }
  ...
  if (scope.typeAnnotation.typeAnnotation.typeName.name !== "GuStack") {
    return context.report(
      ...
      `The first parameter in a construct or pattern constructor must be of type GuStack`
    );
  }

  // 5. Second parameter must be called id
  if (getStandardOrLeftProp(id, "name") !== "id") {
    return context.report(node, id.loc, `The second parameter in a construct or pattern constructor must be called id`);
  }
  ...
  if (getStandardOrLeftProp(id, "typeAnnotation.typeAnnotation.type") !== "TSStringKeyword") {
    return context.report(
      ...
      `The second parameter in a construct or pattern constructor must be of type string`
    );
  }
  ...
  if (props) {
    ...
    if (getStandardOrLeftProp(props, "name") !== "props") {
      return context.report(
        node,
        props.loc,
        `The third parameter in a construct or pattern constructor must be called props`
      );
    }
    ...
    if (getStandardOrLeftProp(props, "typeAnnotation.typeAnnotation.type") !== "TSTypeReference") {
      return context.report(
        ...
        `The third parameter in a construct or pattern constructor must be a custom type`
      );
    }
  }
