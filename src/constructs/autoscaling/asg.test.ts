...
describe("The GuAutoScalingGroup", () => {
  ...
  test("adds any target groups passed through props", () => {
    ...
    Template.fromStack(stack).hasResourceProperties("AWS::AutoScaling::AutoScalingGroup", {
      TargetGroupARNs: [
        {
          Ref: Match.stringLikeRegexp("TargetGroupTesting*"),
        },
      ],
    });
  });
  ...
  test("adds any security groups passed through props", () => {
    ...
    Template.fromStack(stack).hasResourceProperties("AWS::AutoScaling::LaunchConfiguration", {
      SecurityGroups: [
        {
          "Fn::GetAtt": [Match.stringLikeRegexp(`GuHttpsEgressSecurityGroup${app}*`), "GroupId"],
        },
        {
          "Fn::GetAtt": ["WazuhSecurityGroup", "GroupId"],
        },
        {
          "Fn::GetAtt": [Match.stringLikeRegexp("SecurityGroupTesting"), "GroupId"],
        },
        {
          "Fn::GetAtt": [Match.stringLikeRegexp("SecurityGroup1Testing"), "GroupId"],
        },
        {
          "Fn::GetAtt": [Match.stringLikeRegexp("SecurityGroup2Testing"), "GroupId"],
        },
      ],
    });
  });
  ...
  test("does not include the UpdatePolicy property", () => {
    ...
    new GuTemplate(stack).hasResourceWithLogicalId("AWS::AutoScaling::AutoScalingGroup", /AutoscalingGroup.+/);
  ...
  test("overrides the logicalId when existingLogicalId is set in a migrating stack", () => {
    ...
    new GuTemplate(stack).hasResourceWithLogicalId("AWS::AutoScaling::AutoScalingGroup", "MyASG");
  });
  ...
  test("has an instance role created by default with AssumeRole permissions", () => {
    ...
    new GuTemplate(stack).hasGuTaggedResource("AWS::IAM::Role", { appIdentity: { app: "TestApp" } });
