...
export class RiffRaffYamlFile {
  ...
  private constructor(app: App, stacks: GuStack[], riffRaffProjectName?: string) {
  ...
  /**
   * Creates a single `riff-raff.yaml` file for all `GuStack`s in the provided `App`.
   * If not using `GuRoot`, this is the recommended way to instantiate a `RiffRaffYamlFile`.
   */
  public static fromApp(app: App): RiffRaffYamlFile {
    ...
    return new RiffRaffYamlFile(app, allCdkStacks);
  }

  /**
   * Creates a `riff-raff.yaml` file from the provided list `GuStack`s which is likely to be a subset of all stacks in the `App`.
   * This is useful if you want to generate multiple `riff-raff.yaml` files; for example if the repository contains multiple Riff-Raff projects.
   * @param stacks - the stacks to include in the `riff-raff.yaml` file
   ...
   */
  public static fromStacks(stacks: GuStack[], riffRaffProjectName: string): RiffRaffYamlFile {
    if (stacks.length === 0) {
      throw new Error("Unable to produce a working riff-raff.yaml file; there are no stacks!");
    }

    // Check if all the `GuStack`s have the same parent `App`
    const uniqueApps = new Set(stacks.map((stack) => App.of(stack)).filter((app): app is App => app !== undefined));

    if (uniqueApps.size !== 1) {
      throw new Error(
        "Unable to produce a working riff-raff.yaml file; provided stacks do not belong to the same CDK App",
      );
    }

    const app = Array.from(uniqueApps).at(0)!; // We know this is defined because of the previous checks
    return new RiffRaffYamlFile(app, stacks, riffRaffProjectName);
  }
