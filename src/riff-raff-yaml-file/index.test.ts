...
import { GuApiLambda, GuEc2App, GuNodeApp, GuPlayApp, GuScheduledLambda } from "../patterns";
...
describe("The RiffRaffYamlFile class", () => {
  ...
  it("Should include minInstancesInServiceParameters when there are multiple scaling GuEc2AppExperimentals in the same App", () => {
    const app = new App({ outdir: "/tmp/cdk.out" });
    ...
    new MyApplicationStack(app, "test-code-stack", {
      stack: "test",
      stage: "CODE",
      env: { region: "eu-west-1" },
    });

    new MyApplicationStack(app, "test-prod-stack", {
      stack: "test",
      stage: "PROD",
      env: { region: "eu-west-1" },
    });
    ...
    //const cfnParameterName = getAsgRollingUpdateCfnParameterName(codeStack.asg);
    ...
    expect(actual).toMatchInlineSnapshot(`
      ...
      deployments:
        ...
        cfn-eu-west-1-test-my-application-stack:
          ...
          parameters:
            templateStagePaths:
              CODE: test-code-stack.template.json
              PROD: test-prod-stack.template.json
            amiParametersToTags:
              AMIMyapp:
                BuiltBy: amigo
                AmigoStage: PROD
  ...
  it("Should include minInstancesInServiceParameters where relevant when there are multiple stacks in the same App", () => {
    const app = new App({ outdir: "/tmp/cdk.out" });
    ...
    new GuStack(app, "test-simple-stack", {
      stack: "test",
      stage: "PROD",
      env: { region: "eu-west-1" },
    });

    new MyApplicationStack(app, "test-application-stack", {
      stack: "test",
      stage: "PROD",
      env: { region: "eu-west-1" },
    });
    ...
    expect(actual).toMatchInlineSnapshot(`
      ...
      deployments:
        cfn-eu-west-1-test-gu-stack:
          ...
          parameters:
            templateStagePaths:
              PROD: test-simple-stack.template.json
        asg-upload-eu-west-1-test-my-app:
          type: autoscaling
          actions:
            - uploadArtifacts
        ...
        cfn-eu-west-1-test-my-application-stack:
          ...
          parameters:
            templateStagePaths:
              PROD: test-application-stack.template.json
            amiParametersToTags:
              AMIMyapp:
                BuiltBy: amigo
                AmigoStage: PROD
